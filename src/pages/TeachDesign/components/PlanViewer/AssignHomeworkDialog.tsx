...
export default function AssignHomeworkDialog({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (disposed) return;
        const classes: ClassOption[] = Array.from(
          new Map<string, ClassOption>(
            result.data
              .filter(
                (item: any) =>
                  item && typeof item.class_id === "string" && item.class_id,
              )
              .map(
                (item: any) =>
                  [
                  ...
                  ] as [string, ClassOption],
              ),
          ).values(),
        );
        setOptions(classes);
        setClassId(
          classes.some((item) => item.value === defaultClassId)
            ? defaultClassId
            : undefined,
        );
      } catch (error) {
  ...
  const publish = async () => {
    if (
      ...
      loading ||
      loadError ||
      !options.some((item) => item.value === classId)
    )
      return;
  ...
  return (
    <Modal
      ...
      onOk={publish}
      onCancel={() => {
        if (!publishingRef.current) onClose();
      }}
    ...
    >
      ...
      {loading ? (
        ...
        <Alert
          type="info"
          showIcon
          message="暂无可发布的班级，请先添加任教班级"
          action={
            <Button onClick={() => setAttempt((n) => n + 1)}>刷新</Button>
          }
        />
      ) : (
        <>
          ...
          {defaultClassId && (
            <Alert
              ...
              showIcon
              message={
                options.some((item) => item.value === defaultClassId)
