...
export const getPersonalBaseFilterOptions = (questionTypes: any[] = []) => ({
  difficulties: toFilterOptions(
    Object.keys(personalFilterConditions.difficulty_score_map || {}),
  ),
  questionTypes,
  scenes: [],
  abilities: [],
...
export const getFilterOptionsFromJson = (
  ...
  return {
    difficulties: toFilterOptions(
      Object.keys(source.difficulty_score_map || {}),
    ),
...
export const useHeader = () => {
  ...
  const { activeTab, gradeName, subjectName, userSelectionTextbook, filters } =
  ...
  const loadPersonalFilterOptions = useCallback(
    async (courseId?: any) => {
      ...
      const questionTypes = formatXkwQuestionTypeOptions(list).map(
        (item: any) => ({
          value: item.label,
          label: item.label,
        }),
  ...
  const processDictData = (dictData: any) => {
    ...
    dictData?.stage_subject_list?.forEach((stage: any) => {
      // grade_list 现在是字符串数组
      gradeSemestersData[stage.stage_name] =
        stage.grade_list?.map((item: any) => ({
          value: item,
          label: item,
        })) || [];
      // scene_list 现在是字符串数组（从每个学科的第一个获取）
      scenesData[stage.stage_name] =
        stage.subject_list?.[0]?.scene_list?.map((item: any) => ({
          value: item,
          label: item,
        })) || [];
      stage.subject_list?.forEach((subject: any) => {
        ...
        questionTypesData[key] =
          subject?.question_type_list?.map((item: any) => ({
            value: item,
            label: item,
          })) || [];
      });
    ...
    const filterOptions = {
      ...
      difficulties:
        dictData.difficulty_list?.map((item: any) => ({
          value: item.id,
          label: item.name,
        })) || [],
      categories: [
      ...
      uses:
        dictData.use_type_list?.map((item: any) => ({
          value: item.id,
          label: item.name,
        })) || [],
      abilities: [], // 能力 - 暂时为空，等待后端数据
      years:
        dictData.year_list?.map((item: any) => ({
          value: item.id,
          label: item.name,
        })) || [],
      regions:
        dictData.province_list?.map((item: any) => ({
          value: item.id,
          label: item.name,
        })) || [],
  ...
  const initData = async () => {
    ...
    const tabParam =
      new URLSearchParams(history.location.search).get("tab") || "";
    const activeder =
      new URLSearchParams(history.location.search).get("activeder") || "";
    ...
    let finalActiveTab;
    if (
      ["public", "personal", "kgraph", "lesson-plans", "courseware"].includes(
        tabParam,
      )
    ) {
      finalActiveTab = tabParam;
    } else if (activeder) {
