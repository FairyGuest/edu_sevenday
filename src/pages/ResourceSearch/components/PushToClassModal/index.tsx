...
const PushToClassModal = ({
  ...
  // 获取当前课程的目录列表
  const getCourseCatalogList = async (selectedCourseId: string | number) => {
    let payload: any = { space_id: selectedCourseId };
    payload = { ...payload };
    let { code, data }: any = await dispatch({
  ...
  const displayTree = useMemo(() => {
    ...
    groupSubStudentListOption.forEach((cls: any) =>
    ...
    checkedIds.forEach(([, grpId, stuId]) => {
      if (!selMap.has(grpId as string)) selMap.set(grpId as string, []);
      selMap.get(grpId as string)!.push(stuId as string);
    });

    /* 3. 拼树 */
    const tree: any[] = [];
    const clsMap = new Map<string, any>();
    checkedIds.forEach(([clsId, grpId]: any) => {
      if (!clsMap.has(clsId)) {
        const cls = groupSubStudentListOption.find((c: any) => c.id === clsId)!;
      ...
      if (!grpNode.has(grpId)) {
        const grp = groupSubStudentListOption
          .find((c: any) => c.id === clsId)!
          .groups!.find((g: any) => g.id === grpId)!;
