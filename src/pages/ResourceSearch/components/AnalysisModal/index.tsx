...
const getOption = (dataSource: any) => {
  ...
  return {
    ...
    tooltip: {
      ...
      extraCssText:
        "box-shadow: 0 2px 10px rgba(0,0,0,.05); border-radius: 4px;",
...
export default function AnalysisModel(props: any) {
      ...
      () => [
        {
          ...
          align: "center",
          render: (text: string) => (
            <div style={{ color: "#646E8B" }}>{text}</div>
          ),
        },
        {
          title: "数量",
  ...
  const getAnalysisData = async () => {
    ...
    if (code === 200) {
      const { question_type_data, difficulty_data, kpoint_analysis_data } =
        data;
      ...
      setKpointAnalysisData(
        kpoint_analysis_data.map((item: any) => ({
          ...item,
          questions: item.questions.replace(/,/g, "、"),
        })) ?? [],
      );
    }
  };
