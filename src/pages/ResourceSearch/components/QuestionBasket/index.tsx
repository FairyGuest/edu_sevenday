...
export default function QuestionBasket() {
  ...
  const { clearQuestionBasket, getQuestionBasketList, getQuestionBasketCount } =
    useBesket();
  const [searchParams] = useSearchParams();
  const dispatch = useDispatch();
  const id = searchParams.get("id") || "";
  // 从models获取状态
  const { activeTab, questionBasket, questionBasketLoading } = useSelector(
  ...
  const groupedQuestionList = useMemo(() => {
    ...
    let globalIndex = 0;
    return Object.entries(questionGroups).reduce(
  ...
  const onPaperdetail = async () => {
    ...
    if (code === 200) {
      ...
      history.push(
        `/source/resourceSearch/paperDetail?paper_id=${paperid}&editPaper=true&isFromBasket=true&id=${id}`,
      );
      //  clearQuestionBasket();
    }
  };
    ...
    <div className="question-basket-wrap">
      <FloatButton
        ...
        badge={{
          count: questionCount,
          showZero: false,
          overflowCount: all_question_number,
        }}
        description={
          activeTab == "personal" ? "个人题库试题篮" : "公共题库试题篮"
        }
        ...
        footer={
          <Space>
            <Button
              ...
              disabled={questionBasketLoading}
            >
              取消
            </Button>
            <Button
              onClick={() => clearQuestionBasket()}
              disabled={questionBasketLoading || questionCount === 0}
            >
              全部清空
            </Button>
            <Button
              ...
              <Alert
                ...
                message={
                  <div>
                    ...
                    />
                    共计 <b>{questionCount}</b> 道题，平均难度{" "}
                    <b>
                      {getDifficultyLabel(
                        Number(questionBasket.basket_difficulty),
                      )}
                    </b>
                    ，
                    {questionTypeStat.map(
                      (item: any, idx: number, arr: any[]) => (
                        <span key={item.type}>
                          {item.type} <b>{item.count}</b> 道
                          {idx !== arr.length - 1 ? "，" : ""}
                        </span>
                      ),
                    )}
            ...
            <div className="question-basket-list">
              {groupedQuestionList.map((group: any, index: number) => (
                <div key={group.type} className="question-basket-group">
                  <div className="question-basket-group-title">
                    {chineseNumber[index + 1]}. {group.type}
                  </div>
