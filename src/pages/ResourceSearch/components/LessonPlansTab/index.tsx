...
export default function LessonPlansTab() {
  ...
  // 图谱节点命中教案章节（任一选中节点名出现在 chapter 中）
  ...
  return (
    ...
    >
      <Table
        ...
        rowKey="plan_id"
        onRow={(r) => ({
          onClick: () => openDetail(r.plan_id),
          style: { cursor: "pointer" },
        })}
        pagination={false}
        size="small"
        tableLayout="fixed"
        locale={{ emptyText: "当前图谱知识点暂无匹配教案" }}
      />
      <Drawer
            ...
            items={[
              {
                ...
                label: "📖 教案",
                children: (
                  <div className="lp_md_view">
                    <MarkdownRender>
                      {sectionsToMd(detail.sections)}
                    </MarkdownRender>
                  </div>
                ),
              },
              {
                ...
                children: (
                  <div className="lp_md_view">
                    <MarkdownRender>{detail.study_plan_md}</MarkdownRender>
                  </div>
                ),
              },
              {
                ...
                label: (
                  <>
                    <SendOutlined /> 下发记录
                    {detail.issues?.length ? `(${detail.issues.length})` : ""}
                  </>
                ),
                children: detail.issues?.length ? (
