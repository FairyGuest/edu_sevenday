...
const MyQuestionGroup = () => {
  ...
  const { gradeName, subjectName, activeTab } = useSelector(
  ...
  const columns: TableProps<any>["columns"] = [
    ...
    },
    {
      ...
      width: "8%",
      render: (_, record) => (
        <Tag bordered={false} color={difficultyMap[record.difficulty].color}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <span>
          <Button type="link" onClick={() => handlePushToClass(record)}>
            发布
          </Button>
          <Button type="link" onClick={() => handleAnalysis(record)}>
            分析
          </Button>
  ...
  const onedit = (record: any) => {
    const paperid = record?.paper_id;
    history.push(
      `/source/resourceSearch/paperDetail?paper_id=${paperid}&editPaper=${true}`,
    );
  };
  ...
  const deleteGroup = async (record: any) => {
    ...
    if (code === 200) {
      ...
      setDataSource(
        dataSource.filter((item: any) => item.paper_id !== record.paper_id),
      );
      setPagination({
        ...pagination,
