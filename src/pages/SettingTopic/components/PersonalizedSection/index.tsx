...
const PersonalizedSection = ({ classes }: { classes: any[] }) => {
  ...
  const strategyEntries = Object.entries(s.strategy_mix || {}) as [
    string,
    number,
  ][];
  ...
  const loadHomeworkList = async (cid?: string) => {
    ...
    if (!c) return;
    const d = await fetch(`/api/teacher/recommend/homework?class_id=${c}`).then(
      j,
    );
  ...
  useEffect(() => {
    ...
    setPaper(null);
    setStudentId("");
    fetch(`/api/teacher/recommend/homework/${encodeURIComponent(hwId)}`, {
      ...
      .then((d) => {
        ...
        setDetail(d.data);
        setClassId(d.data.class_id);
        setStudentId(d.data.students?.[0]?.sid || "");
      })
      .catch((e) => {
        if (!controller.signal.aborted)
          setLoadError(e.message || "作业加载失败");
      });
    fetch(`/api/teacher/recommend/report?id=${encodeURIComponent(hwId)}`, {
      signal: controller.signal,
    })
      .then(j)
      .then((d) => {
        if (!controller.signal.aborted && d.code === 200) setReport(d.data);
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    if (hwId && studentId) {
      fetch(
        `/api/teacher/recommend/paper-preview?id=${encodeURIComponent(hwId)}&sid=${encodeURIComponent(studentId)}`,
        { signal: controller.signal },
      )
        .then(j)
        .then((d) => {
          if (!controller.signal.aborted && d.code === 200) setPaper(d.data);
        })
        .catch(() => {});
    }
    return () => controller.abort();
    ...
    series: [
      {
        ...
        radius: ["40%", "65%"],
        label: { show: false },
        data: strategyEntries.map(([k, v], i) => ({
          name: k,
          value: v,
  ...
  const generate = async () => {
    ...
    if (!Object.keys(ranges).length) {
      message.warning("至少选择一个布置方针");
      return;
    }
    setGenLoading(true);
    try {
      const cid = classId || classList[0]?.class_id;
      const d = await post("/api/teacher/recommend/generate", {
        class_id: cid,
        ranges,
        total: config.total,
      });
      if (d.code === 200) {
        ...
        setReport(null);
        message.success(
          `已生成 ${d.data.homework_id}（${d.data.summary.n_students}人，每人一单）`,
        );
  ...
  const publish = async (tamper: boolean) => {
    ...
    const d = await post("/api/teacher/recommend/publish", {
      id: hwId,
      student_ids: ids,
    });
    if (d.data?.ok === false) {
      ...
      setDetail((prev: any) => ({
        ...prev,
        status: "published",
        published_at: d.data.published_at,
      }));
      Modal.confirm({
        title: "作业已发布",
        content:
          "学生提交、AI 初批与教师复核进度可在「下发与回收」工作台统一跟踪。",
        okText: "去查看回收进度",
        cancelText: "留在此页",
        onOk: () =>
          history.push(
            `/homework?sub=flow&dispatch_id=${encodeURIComponent("disp-" + hwId)}`,
          ),
      });
    }
  };
  ...
  const paperCols = [
    ...
    { title: "知识点", dataIndex: "cluster", width: 110, ellipsis: true },
    {
      ...
      width: 65,
      render: (t: string) => (
        <Tag color={STRATEGY_TAG[t]?.color || "default"}>
          {STRATEGY_TAG[t]?.label || t}
        </Tag>
      ),
    },
    {
      ...
      ellipsis: true,
      render: (t: string) => (
        <span style={{ fontSize: 11, color: "#8a94a8" }}>{t}</span>
      ),
    },
  ];
        ...
        >
          ...
          <Select
            ...
            options={classList.map((c) => ({
              value: c.class_id,
              label: `${c.class_name} · ${c.n_students}人`,
            }))}
          />
          {(
            [
          ...
          {detail ? (
            <Row gutter={8}>
              ...
              <Col span={6}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 22, fontWeight: 750 }}>
                    {s.q_count?.min}–{s.q_count?.max}
                  </div>
                  <div style={{ fontSize: 11, color: "#8a94a8" }}>题量区间</div>
                </div>
              </Col>
              <Col span={6}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 22, fontWeight: 750 }}>
                    {s.fallback_students ?? 0}
                  </div>
                  <div style={{ fontSize: 11, color: "#8a94a8" }}>兜底策略</div>
                </div>
              </Col>
              <Col span={6}>
            ...
            <Alert
              type="info"
              showIcon
              message="配置方针后点击「生成作业」，或从作业记录中选择历史作业"
            />
          )}
        </Spin>
      </Card>
                ...
                <Select
                  ...
                  options={
                    detail?.students?.length
                      ? detail.students.map((x: any) => ({
                          value: x.sid,
                          label: `${x.name}（${x.n}题）`,
                        }))
                      : Object.entries(detail?.papers || {}).map(
                          ([sid, p]: [string, any]) => ({
                            value: sid,
                            label: `${p.name}（${p.items?.length || 0}题）`,
                          }),
                        )
                  }
                />
          ...
          >
            {detail ? (
              detail.unified_items ? (
                <Table
                  columns={paperCols}
                  dataSource={detail.unified_items}
                  rowKey="qid"
                  pagination={false}
                  size="small"
                  tableLayout="fixed"
                ...
                <Table
                  ...
                  tableLayout="fixed"
                  locale={{
                    emptyText: studentId ? " " : "选择学生后展示其专属试卷",
                  }}
                />
              )
            ) : (
              <Alert
                type="info"
                message="选择作业后可查看（教案/章节作业为全班同卷，个性化作业按学生抽样）"
              />
            )}
          </Card>
        </Col>
          ...
          <Col span={14}>
            <Card size="small" title="📊 作业学情汇总 · 按知识点聚合">
              <Row gutter={8} style={{ marginBottom: 8 }}>
                <Col span={8}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 11, color: "#8a94a8" }}>完成率</div>
                  </div>
                </Col>
                <Col span={8}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 20, fontWeight: 750 }}>
                      {report.overall.required_acc}%
                    </div>
                    <div style={{ fontSize: 11, color: "#8a94a8" }}>
                      必做正确率
                    </div>
              ...
              <p style={{ fontSize: 11, color: "#8a94a8", marginTop: 6 }}>
                回流：{report.ingest_receipt?.accepted || 0}{" "}
                条作答已写入学情事件，可在「学情分析」查看画像更新
              </p>
            </Card>
          </Col>
        ) : null}
