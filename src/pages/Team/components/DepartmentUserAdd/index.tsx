...
const App = (props: any) => {
  ...
  const queryOrgUserList = async () => {
    let payload: any = { org_id: getOrgId(),department_id:currDept?.id}
  ...
  const onOk = async() => {
      ...
      if(selectRowKeys.length>0){
        ...
        let tempArr=tableDataList?.filter((item)=>selectRowKeys.includes(item.id))
        ...
        await dispatch({
          type: 'teamModel/postData',
          apiUrl: "addDepartUserUrl",
          isInfo: true, // api 请求成功提示
          payload: {list: newList,department_id: currDept?.id }
        });
      }
      handleCancel()
  ...
  return <>
    ...
    <span className="add_icon" onClick={() => onClickShowModal()}>{currDept?.user_list?.length || 0}名成员</span>
