...
const FilterBar = ({
  ...
  const init = async () => {
    ...
    const { subject: defaultSubject, questionTypes: defaultQuestionTypes } =
  ...
  const getSubjectList = async () => {
    ...
    if (code == 200) {
      ...
      if (targetSubject) {
        ...
        const curQuestionTypes = targetSubject.children.map(
          (item: any) => item.value,
        );
  ...
  const handleCascaderChange = (value: any) => {
    if (!value || value.length === 0) {
      ...
      onSearch(
        getFilterParams({ subject: undefined, questionTypes: undefined }),
      );
      return;
    }
    ...
    const filteredValue = value.filter(
      (path: any) => path[0] === lastSubjectId,
    );
    setCascaderValue(filteredValue);

    const isParentSelected = filteredValue.some(
      (path: any) => path.length === 1,
    );
    let agentModels: any = undefined;

    if (isParentSelected) {
      const selectedSubjectOption = subjectOptions.find(
        (opt) => opt.value === lastSubjectId,
      );
      if (selectedSubjectOption && selectedSubjectOption.children) {
        agentModels = selectedSubjectOption.children.map(
    ...
    setQuestionTypes(
      agentModels && agentModels.length > 0 ? agentModels : undefined,
    );
  ...
  return (
    ...
    >
      ...
      <div className="list-item-bottom">
          ...
          {item?.question_type === "今日练习" && (
            <div className="practice-info">
              ...
              <Tag
                bordered={false}
                className={
                  Number((item?.correct_rate * 100).toFixed(0)) >= 60
                    ? "success"
                    : "error"
                }
              >
                正确率 {(item?.correct_rate * 100).toFixed(0)}%
              </Tag>
            </div>
          )}
        </div>
        <div className="right">
          <span
            className={`text ${item?.question_type === "今日练习" ? "hidden-text" : ""}`}
          >
            {item?.user_name}
          </span>
          <span className="text time">
            {dayjs(item?.created_at_ts * 1000).format("YYYY-MM-DD HH:mm:ss")}
          </span>
        </div>
      </div>
    </div>
...
const Interaction = () => {
  ...
  const handleSearch = async (filterParams: any) => {
    ...
    if (code == 200) {
      setInteractionList(
        data?.list
          ? Object.values(data.list).reduce(
        ...
        <div className="header-left">
          ...
          {courseId && (
            <ZYIcon
              ...
              onClick={() => filterBarRef.current.onSearch()}
            />
          )}
        </div>
