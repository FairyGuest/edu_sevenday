...
const AgentShop = ({
  ...
  const isInCommon = (app_id: string) =>
    commonAgentList.some((i: any) => i?.app_id === app_id);
        ...
        <div className="agent-list">
          {agentShopList.map((item: any) => (
            <AgentItem
              ...
              type="custom"
              setting={true}
              handleItem={(item: any, type: "remove" | "add") =>
                addAgent(item, type)
              }
            />
          ))}
...
const MyAgent = ({
  ...
  const getMyAgentList = async (currentPage?: number) => {
    ...
    if (code == 200) {
      setMyAgentList(
        data?.applications?.filter((item: any) => item.released === true),
      );
      setTotal(data?.total);
    }
    setLoading(false);
...
const AgentItem = ({
  ...
  const getSpaceInfo = () => {
    return getUserInfo("spaceList").filter(
      (item: any) => item.schoolId == getOrgId(),
    )[0];
  };
  ...
  return (
    <div
      className={`agent-item ${item.disabled ? "disabled" : ""}`}
      onClick={() => {
        addNewTracking({
          bt: "cl",
    ...
    >
      {setting && (
        <div
          className={`handle-btn ${type === "common" ? "jian-btn" : "jia-btn"}`}
        >
          <ZYIcon
            type={type === "common" ? "jian" : "jia"}
      ...
      <div className="agent-item-top">
        ...
        <div className="agent-item-info">
          ...
          <div className="agent-item-meta">
            ...
            {item.app_id && item.source && (
              <span className="meta-divider">|</span>
            )}
            {item.creator && (
              <span className="meta-source">来自 {item.creator}</span>
            )}
          </div>
        </div>
        {/* {item.collect && (
        ...
        <div className="footer-left">
          {item.update_time && (
            <span className="footer-time">
              {dayjs(item.update_time).format("YYYY-M-D")} 更新
            </span>
          )}
...
const Agent = () => {
  ...
  const handleItem = (item: any, type: "remove" | "add") => {
    if (type === "remove") {
      ...
      setCommonAgentList(
        commonAgentList.filter((i: any) => item?.app_id !== i?.app_id),
      );
    } else {
        ...
        {commonAgentList.length > 0 && (
          <div className="agent-header-right">
            {!setting ? (
              <Tooltip title="设置" placement="bottom">
                <Button
                  ...
                  icon={
                    <ZYIcon
                      type="shezhi1"
                      style={{ color: "#646E8B", fontSize: 16 }}
                    />
                  }
              ...
              <Space>
                <Button type="default" onClick={onCancelSetting}>
                  取消
                </Button>
          ...
          <div className="agent-wrap">
            ...
            {setting && (
              <div className="custom-agent-list">
                ...
                />
                <div
                  className={`my-agent-wrap ${activeKey === "myAgent" ? "show" : "hidden"}`}
                >
                  <MyAgent
                ...
                <div
                  className={`agent-shop-wrap ${activeKey === "agentShop" ? "show" : "hidden"}`}
                >
                  <AgentShop
                    handleItem={handleItem}
                    commonAgentList={commonAgentList}
