...
const hasUnhydratedMath = (root: HTMLElement) => {
  ...
  const hasBareKatex = Array.from(root.querySelectorAll(".katex")).some(
    (el) => !el.closest(`.${MATH_WIDGET_CLASS}`),
  );
  if (hasBareKatex) return true;
...
const repairMathWidgetFromDataset = (widget: HTMLElement): boolean => {
  ...
  try {
    ...
    widget.replaceChildren(renderEl);
    return true;
  } catch {
    return false;
  }
...
const extractLatexFromKatexEl = (katexEl: HTMLElement): string => {
  ...
  return (katexEl.textContent || "").replaceAll(CURSOR_ANCHOR, "").trim();
};

const wrapKatexWithWidget = (
  node: HTMLElement,
  latex: string,
  svgUrl = "",
) => {
...
const ensureOrphanKatexWrapped = (root: HTMLElement) => {
  let changed = false;
  Array.from(root.querySelectorAll(".katex")).forEach((katexEl) => {
    const el = katexEl as HTMLElement;
    if (el.closest(`.${MATH_WIDGET_CLASS}`)) return;
    const latex = extractLatexFromKatexEl(el);
    if (!latex) return;
    wrapKatexWithWidget(el, latex);
    changed = true;
  });
  return changed;
};

const repairOrWrapKatexNode = (target: HTMLElement): boolean => {
  const widget = target.classList.contains(MATH_WIDGET_CLASS)
    ? target
    : target.closest<HTMLElement>(`.${MATH_WIDGET_CLASS}`);
  const katexEl = target.classList.contains("katex")
    ? target
    : target.querySelector<HTMLElement>(".katex");

  if (widget?.dataset.latex && repairMathWidgetFromDataset(widget)) {
    return true;
  }

  const latex =
    widget?.dataset.latex ||
    (katexEl ? extractLatexFromKatexEl(katexEl) : "");
  if (!latex) return false;

  if (katexEl && !widget) {
    wrapKatexWithWidget(katexEl, latex);
    return true;
  }

  if (widget) {
    if (!widget.dataset.latex) widget.dataset.latex = latex;
    return repairMathWidgetFromDataset(widget);
  }

  return false;
};

const repairBrokenMathWidgets = (root: HTMLElement) => {
  let changed = ensureOrphanKatexWrapped(root);

  root.querySelectorAll<HTMLElement>(`.${MATH_WIDGET_CLASS}`).forEach((widget) => {
    const katexEl = widget.querySelector(".katex") as HTMLElement | null;
    if ((!katexEl || !isIntactKatex(katexEl)) && widget.dataset.latex) {
      if (repairMathWidgetFromDataset(widget)) changed = true;
    }
  });

  root.querySelectorAll<HTMLElement>(".katex").forEach((katexEl) => {
    if (!isIntactKatex(katexEl)) {
      if (repairOrWrapKatexNode(katexEl)) changed = true;
    }
  });
...
const cleanupEditorDom = (root: HTMLElement) => {
  ...
  Array.from(root.querySelectorAll(".katex")).forEach((katexEl) => {
    ...
    if (!widget || !isIntactKatex(el)) {
      if (widget?.dataset.latex && repairMathWidgetFromDataset(widget)) {
        changed = true;
      } else if (repairOrWrapKatexNode(el)) {
        changed = true;
      } else {
...
const deleteCursorAnchorAtCaret = (
  root: HTMLElement,
  ...
  const removeAnchorNode = (anchor: Node) => {
  ...
  if (startContainer.nodeType === Node.TEXT_NODE && isCursorAnchor(startContainer)) {
    removeAnchorNode(startContainer);
    return true;
  }
  ...
  if (startContainer.nodeType === Node.TEXT_NODE) {
    ...
    if (direction === "before" && startOffset === 0) {
      ...
      if (prev && isCursorAnchor(prev)) {
        removeAnchorNode(prev);
        return true;
      }
    }
    if (direction === "after") {
  ...
  if (startContainer.nodeType === Node.ELEMENT_NODE) {
    ...
    if (direction === "before" && startOffset > 0) {
      const atCaret = el.childNodes[startOffset];
      if (atCaret && isCursorAnchor(atCaret)) {
        removeAnchorNode(atCaret);
        return true;
      }
    }
    const index = direction === "after" ? startOffset : startOffset - 1;
    const anchor = el.childNodes[index];
    if (anchor && isCursorAnchor(anchor)) {
...
const findDeletableTextLeaf = (
  node: Node | null,
  direction: "before" | "after",
): { node: Text; deleteAt: number } | null => {
  if (!node || isCursorAnchor(node) || getRemovableBlock(node)) return null;

  if (node.nodeType === Node.TEXT_NODE) {
    const textNode = node as Text;
    const text = textNode.textContent || "";
    if (direction === "before") {
      let deleteAt = text.length - 1;
      while (deleteAt >= 0 && text[deleteAt] === CURSOR_ANCHOR) deleteAt--;
      if (deleteAt < 0) return null;
      return { node: textNode, deleteAt };
    }
    let deleteAt = 0;
    while (deleteAt < text.length && text[deleteAt] === CURSOR_ANCHOR) {
      deleteAt++;
    }
    if (deleteAt >= text.length) return null;
    return { node: textNode, deleteAt };
  }

  if (node.nodeType !== Node.ELEMENT_NODE) return null;
  const el = node as Element;
  if (el.classList.contains(MATH_WIDGET_CLASS)) return null;

  if (direction === "before") {
    for (let i = el.childNodes.length - 1; i >= 0; i--) {
      const found = findDeletableTextLeaf(el.childNodes[i], direction);
...
const applyTextDeletion = (
  ...
  range.setStart(textNode, deleteAt);
  range.collapse(true);
  selection.removeAllRanges();
  selection.addRange(range);
  return true;
...
const deleteFromElementCaret = (
  range: Range,
  direction: "before" | "after",
): boolean => {
  const { startContainer, startOffset } = range;
  if (startContainer.nodeType !== Node.ELEMENT_NODE) return false;

  const el = startContainer as Element;

  if (direction === "before") {
    if (startOffset === 0) return false;
    const target = el.childNodes[startOffset - 1];
    if (!target) return false;
    if (isCursorAnchor(target)) {
      (target as ChildNode).remove();
      return true;
    }
    const leaf = findDeletableTextLeaf(target, direction);
    if (leaf) return applyTextDeletion(range, leaf.node, leaf.deleteAt);
    return false;
  }
  ...
  if (
    ...
    startOffset === 0 &&
    el.childNodes[0] &&
    (isCursorAnchor(el.childNodes[0]) ||
      getRemovableBlock(el.childNodes[0]) ||
      Boolean(getRemovableBlock(skipCursorAnchors(el.childNodes[0], "next"))))
  ) {
    return false;
  }

  const target = el.childNodes[startOffset];
  if (!target) return false;
  if (isCursorAnchor(target)) {
    (target as ChildNode).remove();
    return true;
...
const deleteCharacter = (
  ...
  if (!range.collapsed) {
    ...
    if (contents.querySelector(`.${MATH_WIDGET_CLASS}, .katex`)) {
      return false;
    }
  ...
  if (startContainer.nodeType === Node.TEXT_NODE && !isCursorAnchor(startContainer)) {
    ...
    if (direction === "before") {
      if (startOffset === 0) return false;
      let deleteAt = startOffset - 1;
      while (deleteAt >= 0 && text[deleteAt] === CURSOR_ANCHOR) deleteAt--;
      if (deleteAt < 0) return false;
      textNode.deleteData(deleteAt, 1);
      range.setStart(textNode, deleteAt);
    } else {
      if (startOffset >= text.length) return false;
      let deleteAt = startOffset;
      while (deleteAt < text.length && text[deleteAt] === CURSOR_ANCHOR) {
        deleteAt++;
      }
      if (deleteAt >= text.length) return false;
      textNode.deleteData(deleteAt, 1);
  ...
  if (startContainer.nodeType === Node.ELEMENT_NODE) {
    return deleteFromElementCaret(range, direction);
  }

  return false;
};

const shouldBlockBrowserDelete = (
  root: HTMLElement,
  direction: "before" | "after",
): boolean => {
  const selection = globalThis.getSelection();
  ...
  if (!range.collapsed) {
    return Boolean(
      range.cloneContents().querySelector(`.${MATH_WIDGET_CLASS}, .katex`),
    );
  }
  ...
  if (
    ...
    const first = (range.startContainer as Element).childNodes[0];
    if (
      first &&
      (isCursorAnchor(first) ||
        getRemovableBlock(first) ||
        getRemovableBlock(skipCursorAnchors(first, "next")))
    ) {
      return true;
    }
  }
...
const getAdjacentRemovableForRoot = (
  ...
  if (!node || isCursorAnchor(node)) return null;

  const removable = getRemovableBlock(node);
  if (
    direction === "after" &&
    startContainer.nodeType === Node.ELEMENT_NODE &&
    startOffset === 0 &&
...
const App = (props: any) => {
  ...
  const getSanitizedHtml = (element: HTMLDivElement) => {
    ...
    clone.querySelectorAll<HTMLElement>(".katex").forEach((katexEl) => {
      if (katexEl.closest(`.${MATH_WIDGET_CLASS}`)) return;
      const latex = extractLatexFromKatexEl(katexEl);
      if (!latex) return;
      const math = document.createElement("math");
      math.setAttribute("latex", latex);
      katexEl.replaceWith(math);
    });
  ...
  const wrapInlineNode = (
    ...
    wrapKatexWithWidget(node, latex, svgUrl);
  };
  ...
  const hydrateMathContent = (container: HTMLElement) => {
    ...
    container.querySelectorAll(".math.inline").forEach((element) => {
      ...
      const existingKatex = el.querySelector<HTMLElement>(".katex");
      if (existingKatex && !existingKatex.closest(`.${MATH_WIDGET_CLASS}`)) {
        const latex =
          extractLatexFromKatexEl(existingKatex) ||
          (el.textContent || "").trim();
        if (latex) wrapInlineNode(existingKatex, latex, "");
        return;
      }
    ...
    ensureOrphanKatexWrapped(container);
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (selectedImage) {
      ...
      if (brokenKatex) {
        if (repairOrWrapKatexNode(brokenKatex)) {
          handled = true;
        } else {
      ...
      } else if (deleteCursorAnchorAtCaret(root, direction)) {
        handled = true;
    ...
    if (handled) {
      ...
      repairBrokenMathWidgets(root);
      isDirtyRef.current = true;
      skipNextInputRef.current = true;
      syncEditorContent({ forceCleanup: false });
    } else if (shouldBlockBrowserDelete(root, direction)) {
      event.preventDefault();
    }
  };
  ...
  const handleEditorInput = () => {
    ...
    repairBrokenMathWidgets(editorRef.current);
    isDirtyRef.current = true;
    syncEditorContent({ forceCleanup: false });
  };
