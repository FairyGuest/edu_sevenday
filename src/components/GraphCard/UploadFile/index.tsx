...
const DraggerUploadFile = (props: any) => {
  const { courseId, multiple = true, onRef, catalogId } = props;
  ...
  const uploadProps: any = {
    ...
    action: `${cogUrl}${props?.fetchFileUrl}`,
    ...
    onChange: async (info: any) => {
      ...
      if (status === "done") {
        ...
        if (code === 200) {
          ...
          props?.onUploadSuccess?.(info, label); // 文件上传成功回调
    ...
    itemRender: (originNode: any, file: any, fileList: any, actions: any) => (
      <>
        ...
        <div key={file.uid} className="file_item">
          <div className="file_item_name">{handleFileName(file.name)}</div>
  ...
  const handleFileName = (name: string) => {
    const math = name.match(/\.[^/.]+$/);
    if (math) {
      switch (math[0]) {
        case ".pdf":
          return (
            <>
              <ZYIcon type="pdf-color" className="pdf" />
              <span>{name}</span>
            </>
          );
        case ".doc":
