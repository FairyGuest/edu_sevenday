 ...
 * 原则（I6 执行安全）：
 *  - 只读/跳转类直接执行；写操作（生成作业/下发）由确认卡+Modal 二次确认后执行；
 *  - 生成个性化作业为真实调用 mock 推荐引擎（POST /teacher/recommend/generate）。
 * 页面流转（v2.0 优化）：动作**不直接跳转**，返回 navigateTo 目标；
 * 由 GlobalAssistant 统一"先收起面板 → 下一帧再切路由"，
 * 避免新页面在遮罩下挂载（重组件 mount + 数据拉取被压住）造成的"卡一下"。
 */
...
export async function runAction(
  ...
  if (key === "personalized_paper" || key === "assign_homework") {
    const strategy = STRATEGY_RANGES[params.strategy] ? params.strategy : "weak";
    const classId = params.class_id;
    try {
      const r = await fetch("/api/teacher/recommend/generate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ class_id: classId, ranges: STRATEGY_RANGES[strategy], total: 6 }),
      });
